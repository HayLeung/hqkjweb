...
function pieChart() {
  ...
  // Config settings
  var chartSizePercent = 55;                        // 饼型大小The chart radius relative to the canvas width/height (in percent)
  var sliceBorderWidth = 1;                         // Width (in pixels) of the border around each slice
  var sliceBorderStyle = "#fff";                    // Colour of the border around each slice
  //var sliceGradientColour = "#999";                 // Colour to use for one end of the chart gradient
  var maxPullOutDistance = 25;                      // How far, in pixels, to pull slices out when clicked
  var pullOutFrameStep = 4;                         // 速度 How many pixels to move a slice with each animation frame
  var pullOutFrameInterval = 40;                    // How long (in ms) between each animation frame
  var pullOutLabelPadding = 45;                     // 文字距离Padding between pulled-out slice and its label  
  ...
  var pullOutBorderWidth = 0;                       // 线宽 Width (in pixels) of the pull-out slice border
  ...
  function init() {
    ...
    centreX = canvasWidth / 2.2;
    centreY = canvasHeight / 2;
    chartRadius = Math.min( canvasWidth, canvasHeight ) / 1.8 * ( chartSizePercent / 100 );
